@extends('layouts.admin-lte.blank',['folder' => 'edit'])
...
function SEND(request)
{
	...
	if(!error)
	{
		...
		//var mail_old_attachments = $('input[name="mail_old_attachments\\[\\]"]').map(function(i, elem) { return $(this).val(); }).get();
		//formData.append("mail_old_attachments[]",mail_old_attachments);
		for (var x = 0; x < $('input[name="mail_old_attachments\\[\\]"]').length; x++) {
			formData.append("mail_old_attachments[]",$('input[name="mail_old_attachments\\[\\]"]').eq(x).val());
			//alert($('input[name="mail_old_attachments\\[\\]"]').eq(x).val());
		}
            ...
            <div class="box-body">
              <div id="div-mail_from" class="form-group">
                  ...
                  @foreach($mail_accounts as $mail_account)
                    <option value="{{ $mail_account->id }}" {{ $mail_account->email == $mail_email->sender ? "selected" : ""}}>{{ $mail_account->name }} &lt;{{ $mail_account->email }}&gt; </option>
                  @endforeach
                  </select>
              </div>
              <div id="div-mail_to" class="form-group">
                <input class="form-control" name="mail_to" id="mail_to" value="{{ $mail_email->recipient }}" placeholder="To:">
              </div>
              <div id="div-mail_subject" class="form-group">
                <input class="form-control" name="mail_subject" id="mail_subject" value="{{ $mail_email->subject }}" placeholder="Subject:">
              </div>
              <div id="div-mail_content" class="form-group">
                    <textarea id="mail_content" name="mail_content" class="form-control" style="height: 300px">{{ $mail_email->body_plain }}</textarea>
              </div>
			  <div class="form-group">
              	<ul class="mailbox-attachments clearfix">
                @foreach($mail_email->mail_attachments as $attachment)
