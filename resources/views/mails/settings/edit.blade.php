...
function UPDATE()
{
	...
	if(!error)
	{
		...
		type: 'PUT',
		url: '{{ route('mail_settings.update',['setting' => $result->id ]) }}'
		}).done(function( data ) {
			if(data=="")
			{
...
<div class="box  box-primary col-md-12">
	...
	<div class="box-body">
        ...
        <div class="form-group row">
			...
			<input type="text" id="name" name="name" value="{{ $result->name }}" class="form-control" placeholder="{{ __('Name') }}" autocomplete="off">
					</div>
				</div>
		</div>
			...
			<label for="email" class="col-sm-1 col-form-label">{{ __('Email') }}</label>
				<div class="col-sm-8">
					<div id="div-email" class="form-group">
						<input type="email" id="email" value="{{ $result->email }}" name="email" class="form-control" placeholder="{{ __('Email') }}" autocomplete="off">
					</div>
